...
public class DataConvert {
	 ...
	 * @param b: byte array
	 * @param length: valid length of b
	 * @param bytesPerInt: number of bytes per Int
	 * @return
	 */
	public static int[] byteArr2IntArr(byte[] b, int length, int bytesPerInt) {
		if (length % bytesPerInt != 0) {
			System.out.printf("byte array length is not apllicable.\n", length);
			System.out.println("length: "+ length + "\t bytesPerInt: " + bytesPerInt);
			return null;
		}
...
//				System.out.println("bin:" + Integer.toBinaryString(Integer.MIN_VALUE >> (4-bytesPerInt)*8));
//				System.out.println("bin:" + Integer.toBinaryString(Integer.MIN_VALUE));	//符号位为1，其余为0
//				System.out.println("0x80 & (1<<7): " + (0x80 & (1<<7))); //结果为0x80(128)，不为1
	 ...
	 * @param b: byte array
	 * @param numPerDouble: number of bytes per Double
	 * @param fractionNum: the bit's number of fraction
	 * @param length: valid length of b
	 * @return
	 */
	 ...
	 * @param length: valid length of double array
	 * @return
	 */
	...
	public static void main(String[] args) {
		...
		int[] ia = byteArr2IntArr(ba, ba.length, 3);
		for (int i : ia)
			System.out.format("%x \n", i);	//0x:665544, 332211
